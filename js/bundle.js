...
/* harmony export */   "mult": function() { return /* binding */ mult; },
...
function cards(params) {
  ...
  axios.get('http://localhost:3000/menu').then(data => {
    ...
    // ЕСЛИ НЕТ СОЕДИНЕНИЯ И Т.Д
    // ООП

    new MenuCart('img/tabs/vegy.jpg', 'vegy', "Меню 'Фитнес'", "Меню 'Фитнес' - это новый подход к приготовлению блюд: больше свежих овощей и фруктов. Продукт активных и здоровых людей. Это абсолютно новый продукт с оптимальной ценой и высоким качеством!", 30, '.menu .container').render();
    new MenuCart('img/tabs/elite.jpg', 'elite', "Меню 'Премиум'", "В меню 'Премиум' мы используем не только красивый дизайн упаковки, но и качественное исполнение блюд. Красная рыба, морепродукты, фрукты - ресторанное меню без похода в ресторан!", 40, '.menu .container', 'menu__item', 'big').render();
    new MenuCart('img/tabs/post.jpg', 'post', "Меню 'Постное'", "Меню 'Постное' - это тщательный подбор ингредиентов: полное отсутствие продуктов животного происхождения, молоко из миндаля, овса, кокоса или гречки, правильное количество белков за счет тофу и импортных вегетарианских стейков.", 40, '.menu .container', 'menu__item').render();
  });
}
...
function slider(_ref) {
  ...
  const {
    width
  } = window.getComputedStyle(slidesWrapper); // узнаем ширину видимой части слайдера
  ...
  next.addEventListener('click', () => {
    if (offset === deleteNotDigits(width) * (slides.length - 1)) {
      // удаляем px с помощью регулярных
      offset = 0;
    } else {
...
function timer(id, deadLine) {
  function getTimeRemaining(endtime) {
    ...
    let hours;
    let minutes;
    let seconds; // для того чтобы вставить 0 если отрицательная t
...
const postData = async (url, data) => {
  // async означает что будет асинхронный код
  const res = await fetch(url, {
    // отправить на сервер. await всегда идет с async и показывает
...
console.log((0,_modules_calcul__WEBPACK_IMPORTED_MODULE_1__.mult)(3, 4));
console.log((0,_modules_calcul__WEBPACK_IMPORTED_MODULE_1__.sum)(3, 4));
window.addEventListener('DOMContentLoaded', () => {
  const modalTimerId = setTimeout(() => (0,_modules_modal__WEBPACK_IMPORTED_MODULE_4__.openModal)('.modal', modalTimerId), 50000); // показать модалку через 50сек
  (0,_modules_tabs__WEBPACK_IMPORTED_MODULE_2__["default"])('.tabheader__item', '.tabcontent', '.tabheader__items', 'tabheader__item_active');
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_3__["default"])('.timer', '2023-06-30');
