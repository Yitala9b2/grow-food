...
<div  id="ladesign_description" align="center" >
  ...
  <p>В данной работе с нуля были написаны слайдер с пагинацией и счетчиком, таймер с обратным отсчетом, попапы с формой (также их отправка в базу json.db с помощью xhr, fetch),
   ...
   для работы с базой использовался json-server) если json-server не активен и подгрузка json не возможна срабатывает catch() и рендерятся экземпляры класса прописаные в файле js. Так как это мой тестовый проект, точечная стилизация и медиа запросы не брались во внимание
  </p>
